...
class MADNet2(nn.Module):
    ...
    def training_loss(self, pred_disps, gt_disp):
        ...
        loss = 0.005 * F.l1_loss(pred_disps[0], -F.interpolate(gt_disp, scale_factor=1 / 4., mode='nearest') / 20.,
                                 reduction='sum') + \
               0.01 * F.l1_loss(pred_disps[1], -F.interpolate(gt_disp, scale_factor=1 / 8., mode='nearest') / 20.,
                                reduction='sum') + \
               0.02 * F.l1_loss(pred_disps[2], -F.interpolate(gt_disp, scale_factor=1 / 16., mode='nearest') / 20.,
                                reduction='sum') + \
               0.08 * F.l1_loss(pred_disps[3], -F.interpolate(gt_disp, scale_factor=1 / 32., mode='nearest') / 20.,
                                reduction='sum')
        return loss
    ...
    def compute_loss(self, image2, image3, predictions, gt, validgt, adapt_mode='full', idx=-1):
        ...
        if adapt_mode == 'full':
            ...
            self.accumulated_loss += torch.stack([loss[i] * self.loss_weights[i] for i in range(len(loss))],
                                                 0).detach().cpu()
            loss = sum(loss).mean()
